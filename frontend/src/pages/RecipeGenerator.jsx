...
const RecipeGenerator = () => {
            ...
            try {
                ...
                if (isMounted && preferences) {
                    setDietaryRestrictions(prev => 
                        prev.length === 0 ? preferences.dietary_restrictions || [] : prev
                    );

                    setCuisineType(prev => 
                        prev === 'Any' ? (preferences.preferred_cuisines?.[0] || 'Any') : prev
                    );

                    setServings(prev => 
                        prev === 4 ? (preferences.default_servings || 4) : prev
                    );
                }
            } catch (error) {
                console.error(error);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setGeneratedRecipe(res.data.data.recipe)
            toast.success("Recipe Generated Successfully")
        } catch (error) {
            console.error(error)
            toast.error("Failed To Generate Successfully")
        } finally {
            setGenerating(false)
        }
    };
    ...
    const handleSaveRecipe = async () => {
        ...
        try {
            ...
            toast.success("Recipes Saved To Your Collection")
        } catch (error) {
            console.error(error)
